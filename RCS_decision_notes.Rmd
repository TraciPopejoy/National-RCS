...
library(magick)
library(Hmisc, verbose=F, quietly = T)  
```
...
huc12_crop<-huc12_crop%>% 
  filter(states!="MX", states!="CN",
         !(name %in% c("Lake Superior","Lake Erie","Lake Ontario",
...
```{r mexico huc12 prism raster plot, echo=F, fig.width=6, fig.height=3.5}
...
rm(raster.points, hc12_crop_t, hc12_crop_t_m,ppt_mean_usa)
```
...
usa_l48_alb<-st_transform(x=usa_l48, crs=crs.albers)
...
rm(dat_sp, dat_sp_usa, d1km_buff_usa, dissolved_1km_buffer)
...
```{r range barely in US plots, echo=F, fig.width=3, fig.height=3, fig.align='center'}
ggplot()+
  ...
  theme_bw()+coord_flip()
```
```{r range barely in US sp, echo=F, fig.width=6, fig.height=4, fig.align='center'}
...
```{r Smilisca baudinii range, echo=F, fig.height=4, fig.height=4}
...
ggplot()+geom_sf(data=usa_l48_alb)+
  ...
  geom_sf(data=sb_dat_sp_usa, color="red", alpha=0.3)+
    coord_sf(xlim=st_bbox(sb_dat_sp)[c(1,3)],
           ylim=st_bbox(sb_dat_sp)[c(2,4)])+
  ggtitle("Observations of Smilisca baudinii")+theme_bw()
rm(sb_dat_sp, sb_dat_sp_usa)
```
 

```{r dryophytes eximius ex, echo=F, fig.height=4, fig.height=4}
...
ggplot()+geom_sf(data=usa_l48_alb)+
  ...
  geom_sf(data=de_dat_sp_usa, color="red", alpha=0.3)+
    coord_sf(xlim=st_bbox(de_dat_sp)[c(1,3)],
           ylim=st_bbox(de_dat_sp)[c(2,4)])+
  ggtitle('Observations of Dryophytes eximius')+theme_bw()
rm(de_dat_sp, de_dat_sp_usa)
```
...
I am only using the 1 km buffers for comparability. Below we explore the compatability of HydroBASINS datasets, which are international watersheds delineated by elevation, and USGS HUC12s. This represents the subset of taxa I chose to calculate the RCS at the contiguous US extent and the North American continent extent with the WorldClim climate rasters. I will also compare this with the RCS calculated at the contiguous US extent with the PRISM climate raster data.
...
anuran.taxa.sub<-c('Dryophytes eximius','Dryophytes wrightorum',
                   ...
                   "Pseudacris nigrita", "Dryophytes chrysoscelis",
                    "Incilius nebulifer", "Anaxyrus nelsoni",
                    "Incilius valliceps", "Rana draytonii")
rmarkdownTable(tibble(`Scientific Name`=anuran.taxa.sub,
                      `Reason for inclusion`=c('Large range, rare in US',
                                               ...
                                               rep('.Random species subsampled', 6))) %>%
                 arrange(desc(`Reason for inclusion`)))
```

The below code chunk calculates the area of occupancy at two different spatial extents. It writes out two shapefiles of a single multipolygon that represents the AOO at different spatial extents. 
...
for(i in 1:length(anuran.taxa.sub)){
  ...
  # Generate 1km point buffers. You must use a projected CRS for function gBuffer(). 
  #For CRS, we use: USA Contiguous albers equal area. 
  dat_sp <- st_transform(dat_sp, crs.albers) #overwrites to the new projection (save on memory)
...
Below is the AOO for our subset of taxa. The line represents no difference in the scaled AOO between the two spatial extents. If a species is within top left portion of the graph, using the contiguous US spatial extent overestimates the rarity of the species (meaning the AOO is actually larger at the continental spatial extent). Species in the bottom right half of the graph have their rarity underestimated (strictly because the scale is relative and species within the top left alter that scale).  

```{r AOO range results plot, echo=F, fig.width=4, fig.height=4}
ggplot() + 
  geom_abline(slope=1, intercept=0)+
  geom_point(data=subRCS, 
             aes(x=aoo_na_scaled, y=aoo_us_scaled), alpha=0.5)+
  geom_text_repel(data=subRCS,
...
The code below is used to extract WorldClim data (~21.5 km2 spatial resolution) at the contiguous US and North American continent spatial extent. We also extract the PRISM climate data (~4 km spatial resolution) at the contiguous US spatial extent. 

```{r evaluating climate niche breadth, eval=FALSE}
...
Below, I calculate the climate niche breadth and then scale it for comparability among different taxa. 

```{r skipping climate niche quant}
# WorldClim data at two spatial extents
subCS<-read.csv('/home/tracidubose/WorldClim_data/Subset_ClimateNiche_Output.csv') %>%
  dplyr::select(-X)
sub_CS_calc<-subCS %>%
...
# PRISM data at the contiguous US extent
prismCS<-read.csv('/home/tracidubose/rcs_results/prism_subset_cs.csv') %>%
  pivot_longer(-species) %>%
  rename(variable='name', sd_of_env_mean='value') %>%
  group_by(variable)%>% 
  #calculating the index
  mutate(var_CS_index = (sd_of_env_mean-min(sd_of_env_mean))/ 
                        (max(sd_of_env_mean)-min(sd_of_env_mean)),
         CS_adj=1-var_CS_index) %>%
  ungroup()%>%
  mutate(variable=recode(variable, ppt_sd='annual_ppt', temp_sd='mean_temp'))
```

```{r plotting climate niche, echo=F}
worldclim_gdf<-sub_CS_calc %>% 
  pivot_longer(cols=c(annual_ppt, mean_temp), names_to = 'variable') %>%
  dplyr::select(scientific_name, spatial_extent_cat, variable, value) 

ggplot()+
  geom_point(data=prismCS, aes(x=species, y=CS_adj, color="PRISM L48"), size=3, alpha=0.6)+
  geom_point(data=worldclim_gdf[worldclim_gdf$spatial_extent_cat=="North America",],
             aes(x=scientific_name, y=1-value, color="WorldClim NA"), size=3, alpha=0.6)+
  geom_point(data=worldclim_gdf[worldclim_gdf$spatial_extent_cat=="Lower 48",],
             aes(x=scientific_name, y=1-value, color="WorldClim L48"), size=3, alpha=0.6)+
  scale_color_viridis_d(name="Dataset & Extent")+
  scale_y_continuous(name='Scaled Climate Niche Breadth')+
  facet_wrap(~variable)+
  theme_bw()+
...
I calculated the AOO and CS at two different spatial extents using two different climate data sources for a subset of the Anuran taxa that occur within the US. This gives us three different RCS values to calculateThe grain size for this analysis is currently only 1km buffered points.

```{r calculate the RCS for each dataset}
subRCS_PRISM<-prismCS %>% dplyr::select(species, variable, CS_adj) %>%
  pivot_wider(names_from='variable', values_from='CS_adj') %>%
  rowwise() %>%
  mutate(spatial_extent_cat="Prism Lower 48",
         buff_CS=mean(c(annual_ppt, mean_temp))) %>% #already adjusted
  left_join(subRCS, by=c('species'='scientific_name')) %>%
  rename(scientific_name='species', aoo_buff="aoo_us_scaled") %>%
  mutate(RCS_buff=(aoo_buff+buff_CS)/2) %>%
  dplyr::select(scientific_name, RCS_buff, buff_CS, aoo_buff, aoo_usal48, spatial_extent_cat)
  
subRCS_WorldClim<-subRCS %>% 
...
subRCS_all<-bind_rows(subRCS_PRISM, subRCS_WorldClim)
```

```{r RCS results plot, echo=F, fig.width=4, fig.height=8, fig.align='center'}
library(ggrepel)
subRCS_gdf<-subRCS_all %>%
  dplyr::select(scientific_name, spatial_extent_cat, RCS_buff) %>%
  pivot_wider(names_from = spatial_extent_cat, values_from = RCS_buff)
wc_extent<-ggplot(data=subRCS_gdf, 
             aes(x=`North America`, y=`Lower 48`)) + 
  geom_abline(slope=1, intercept=0)+
  geom_point()+
  geom_text_repel(aes(label=scientific_name), size=3)+
  scale_x_continuous("RCS in N American Continent")+
  scale_y_continuous("RCS in Contiguous US")+
  ggtitle('Effect of spatial extent')+
  theme_bw()
wc_v_p<-ggplot(data=subRCS_gdf, 
             aes(x=`Prism Lower 48`, y=`Lower 48`)) + 
  geom_abline(slope=1, intercept=0)+
  geom_point()+
  scale_x_continuous("RCS using PRISM")+
  scale_y_continuous("RCS using WorldClim")+
  ggtitle('WorldClim vs. PRISM')+
  theme_bw()
plot_grid(wc_extent, wc_v_p, ncol=1)
```

Based on the above graphs, I don't think data set matters too much. But, spatial extent matters a lot. I would argue we should use the entire range of the species to calculate the RCS for this product. The problem occurs that HUC12 don't align well with HydroBasins. 

# How does HydroSHEDS dataset match up with USGS HUCS?

We want to use HUCS because they are a management unit, have ecological significance, and are less sensitive to sample size. But HUCS are not delineated outside major watersheds that flow through the US. This means if we used HUCs to collate occurrences, parts of Mexico and Canada would not have the polygons to capture the occurrences. We could potentially use HydroBASINS, which represent watersheds and sub-watersheds:  <https://hydrosheds.org/page/hydrobasins>. The following code investigates if there is congruence between USGS HUC12s and the HydroBASINS polygons

```{r load a hydrosheds and get kiamichi subset}
hydro12<-st_read(dsn=PATH_hydrosheds, layer='hybas_na_lev12_v1c', quiet = T)

# Isolating a smaller area for comparison (to save computation time)
ok_kiamichi<-huc12_crop %>% filter(states=="OK",
                      substr(huc12, 1,8)==11140105)
ok_kiamichi_t<-st_transform(ok_kiamichi, st_crs(hydro12))

# identify HydroBASINS that intersect with the Kiamichi HUC12s
hydro12_kiamichi_int<-st_intersection(ok_kiamichi_t, hydro12)
hydro12_kiamichi<- hydro12_kiamichi_int %>%
  pull(HYBAS_ID) %>% unique()
```

```{r overlap between hydroBasins and usgshucs, echo=F}
plot_grid(ggplot()+geom_sf(data=hydro12[hydro12$HYBAS_ID %in% hydro12_kiamichi,])+
            ggtitle('HydroSHED level 11')+theme_bw(),
  ggplot() + geom_sf(data=ok_kiamichi_t, fill="blue", alpha=0.5)+
    ggtitle('USGS HUC 12')+theme_bw(),
  ggplot()+geom_sf(data=hydro12[hydro12$HYBAS_ID %in% hydro12_kiamichi,])+
    geom_sf(data=ok_kiamichi_t, fill="blue", alpha=0.5)+ggtitle('Combination')+theme_bw(),
  nrow=1)
```

How does this translate to when we isolate one HydroBASIN polygon? 

```{r one hydrobasin plot, echo=F, fig.width=5, fig.height=4}
# specifically looking at one polygon from the HydroShed level 11
int.hybas<- hydro12_kiamichi_int %>%
  count(HYBAS_ID) %>%
  arrange(desc(n)) %>%
  slice(3) %>% 
  pull(HYBAS_ID)
int.hucs<- hydro12_kiamichi_int %>%
  filter(HYBAS_ID==int.hybas) %>%
  pull(huc12) %>% unique()
ggplot() + 
  geom_sf(data=ok_kiamichi_t[ok_kiamichi_t$huc12 %in% int.hucs, ], alpha=.5, aes(fill='HUC 12'))+
  geom_sf(data=hydro12[hydro12$HYBAS_ID==int.hybas,], alpha=.5, aes(fill='HydroBASINS')) + 
  scale_fill_viridis_d(name="Polygon\nDataset")+
  theme_bw()
```

The best solution might be to peice the two datasets together - keep HUC12 in the contiguous US while using Hydroshed level 11 throughout the rest of North America. The HUC12s have an average area of `r mean(ok_kiamichi_t$areasqkm)` square kilometers (SD = `r sd(ok_kiamichi_t$areasqkm)`), while the HydroSHED polygons have an average area of `r mean(hydro12[hydro12$HYBAS_ID %in% hydro12_kiamichi,]$SUB_AREA)` square kilometers (SD =  `r sd(hydro12[hydro12$HYBAS_ID %in% hydro12_kiamichi,]$SUB_AREA)`). This would mean that the area outside of the US would be inflated and the CS values would be weighted higher. I don't really like this as it would downweight the US populations which might imply the in those areas, Mexico and Canada have more of a responsibility to conserve those species (because the RCS will be lower). 

```{r removing large files, echo=F}
rm(ok_kiamichi, ok_kiamichi_t, hydro12_kiamichi)
```
...
```
```{r time plots, echo=F, fig.width=6, fig.height=4}
...
rmarkdownTable(bind_rows(g_car_hm %>% mutate(Source='HerpMapper'),
          g_car_gbif%>% mutate(Source='GBIF')) %>%
            arrange(desc(n)))
```
```{r g car plot, echo=F, fig.height=4.5, fig.width=4.5}
ggplot()+
...
# Native vs Entire Range for calculating climate breadth niche

So we noticed some of the observation for our anurna taxa were well outside the 'native' range of the species. Restricting use to the native range definitely alters the area of occupancy. But, if occurrences only occur in areas the frog can survive, the climate niche breadth shouldn't be altered too much. In the code below I quantify how many points occur outside the native range for that anuran. I also quantify the climate niche breadth on a subset of taxa to evaluate how using the native vs. entire range alters the climate niche breadth.    

```{r count obs in and outside IUCN native range, warnings=FALSE, message=FALSE}
#loading native ranges from IUCN
iucn_anuran<-read_sf('/home/tracidubose/ANURA/', 'ANURA')
#loading all the Anuran observations  
Anurans_sf<-read.csv(PATH_AllAnuran_OccurrenceData, stringsAsFactors = F) %>%
  st_as_sf(coords=c('Longitude','Latitude'), crs=crs.geo) 
#st_crs(iucn_anuran)==st_crs(Anurans_sf)

range_obs<-data.frame(final.taxa=character(),
                      in_range=numeric(),
                      out_range=numeric(),
                      total_obs=numeric())
for(y in unique(Anurans_sf$final.taxa)){
  pts_inside<-st_intersects(st_union(iucn_anuran[iucn_anuran$binomial==y,]),
              Anurans_sf[Anurans_sf$final.taxa==y,], sparse = F)
  range_obs<-bind_rows(range_obs,
                       data.frame(final.taxa=y,
                       in_range=sum(pts_inside),
                       out_range=length(pts_inside)-sum(pts_inside),
                       total_obs=nrow(Anurans_sf[Anurans_sf$final.taxa==y,])))
}
```

This table reports the taxa with the lowest proportion of observations within their native range.

```{r pts inside native range, echo=F, results='asis'}
rmarkdownTable(range_obs %>% 
       mutate(sum_pts=in_range+out_range,
              per_inside=round((in_range/sum_pts)*100,2)) %>% 
         arrange(per_inside) %>%
         rename(Taxa='final.taxa',
                'Pts in native range'='in_range',
                'Pts outside native range'='out_range', 
                'Total Obs.'='total_obs', 
                'Percent in Native Range'='per_inside') %>%
         dplyr::select(-sum_pts) %>%
         slice(3:10))
#range_obs %>%  mutate(sum_pts=in_range+out_range,
#              per_inside=round((in_range/sum_pts)*100,2)) %>% 
#  arrange(per_inside) %>% slice(5:10) %>% pull(final.taxa)
```

So, I wanted to use a subset of the taxa to investigate how the quantification of the climate niche breadth might shift by using the native range vs. entire range. This subset included the following taxa: `r paste(range_obs %>% arrange(desc(out_range))  %>% slice(4:8) %>% pull(final.taxa), sep=', ')`. I used code from the ARC_Climate Sensitivity_Anurans.R and the *extract_mean_env_buff* function to extract the climate niche breadth of each taxa in their native range using their 1km buffers. 


```{r climate niche differences, echo=F}
clim_values<-NULL
for(b in list.files('/home/tracidubose/1008_nat_rangeinv/', full.name=T)){
  ms<-read.csv(b)
...
aoo_nat<-NULL
iucn_anuran<-st_transform(iucn_anuran, crs.albers)
for(u in unique(clim_values$species)){
  sp_data<-Anurans_sf %>% filter(final.taxa == u) %>%
    st_transform(crs.albers) %>%
    st_buffer(1) %>% st_union()
  all_area<-st_area(sp_data)
  nat_range<-st_intersection(iucn_anuran[iucn_anuran$binomial==u,], sp_data)
  nat_area<-st_area(st_union(nat_range))
  aoo_nat<-bind_rows(aoo_nat, 
                     data.frame(final.taxa=u,
                                total.area=as.numeric(all_area),
                                nat.area=as.numeric(nat_area)))
}

sp_ord <- aoo_nat %>% filter(final.taxa %in% clim_values$species)  %>% 
  mutate(per=nat.area/total.area) %>%
  arrange(per) %>%
  pull(final.taxa)

aoo_nat<-aoo_nat %>% rowwise() %>%
  mutate(ext.area=total.area-nat.area) %>%
  pivot_longer(cols=c('nat.area', 'ext.area')) %>%
  mutate(percent.range=(value/total.area)*100,
         spFAC=factor(final.taxa, levels=sp_ord))
```

The code below reads in the results of the extract function and calculates the standard deviation of each climate variable (ppt, minimum temperature, and maximum temperature) for the subset of taxa. 

```{r}
#reading in area of occupancy in the lower 48 and elsewhere to adjust weights.
#This means climate values are adjusted based on the area those values occupy.
aoo_usa<-read.csv('/home/tracidubose/rcs_results/AOOs_raw_202009.csv') %>% 
  dplyr::select(-X, -DFdif) %>%
  #only looking at the point buffers
  filter(area.type %in% c('usa_l48_alb_1km',
                          'NOTusa_l48_alb_1km')) %>% 
  group_by(scientific_name) %>%
  #calculating the proportion that should be represented by prism vs. worldclim values
...
buf_climate_sum<-clim_values %>% 
  ...
  filter(!is.na(value)) %>%
  group_by(species, source, name) %>%
  #creating this column to join it with the aoo_usa dataframe
  mutate(area.type=case_when(source=='prism'~'usa_l48_alb_1km',
                             source!='prism'~'NOTusa_l48_alb_1km')) %>%
  left_join(aoo_usa, by=c('species'='scientific_name', 'area.type')) %>%
  #creates the new weights that will sum to 1 (other than NA cells)
  mutate(new.weight=weight*prop.area) %>%
  group_by(species, name) %>%
  # this code calculates the weighted mean and the weighted sd value 
  # weights are from areas. 
...
climate_niche<-buf_climate_sum %>%
  # join in previously calculated entire range climate niche breadth values
  left_join(read.csv('/home/tracidubose/rcs_results/watershed_climate_means/range.wide.weighted.CS_20200929.csv') %>%
              # Entire Range == ER
              rename('ER_mean_value'='mean_value', 
                     'ER_sd_value'='sd_value'),
            by=c('species','name')) %>%
  dplyr::select(-X, -sumweight) %>%
  pivot_longer(cols = c(mean_value, ER_mean_value,
                        sd_value, ER_sd_value), 
               names_to = 'mean_type', 
               values_to = 'value') %>%
  # order species based on highest proportion within native range
  mutate(spFAC=factor(species, levels=sp_ord))

CS_total<-climate_niche %>%
  filter(grepl('sd_value', mean_type)) %>%
  group_by(name, mean_type) %>%
  mutate(CS_value=(value-min(value))/ #calculating the index
           (max(value)-min(value))) %>%
  group_by(spFAC,species, mean_type) %>%
  dplyr::summarize(CS=mean(CS_value))
```

```{r sd btwn nat ext range, echo=F}
sd.plot<-ggplot()+geom_point(data=climate_niche[climate_niche$mean_type %in% c('sd_value', 'ER_sd_value'),],
                    aes(x=value, y=spFAC, color=mean_type), alpha=0.5, size=2)+
  facet_wrap(~name, scales='free_x')+
  scale_y_discrete('') +
  scale_x_continuous('sd of climate')+
  scale_color_viridis_d('', labels=c('entire range','natural range'), end=.8)+
  theme_bw()+theme(legend.position = 'top',
                        axis.text.y = element_text(face='italic', size=8),
                        axis.text.x= element_text(angle = 20, size=8))+
  guides(color=guide_legend(override.aes=list(alpha=1)))
CS.plot<-ggplot()+
  geom_point(data=CS_total, aes(x=1-CS, y=spFAC, color=mean_type), alpha=0.5)+
  facet_wrap(~'CS Value')+
  scale_y_discrete('') +
  scale_color_viridis_d('', labels=c('entire range','natural range'), end=.8)+
  theme_cowplot()+ theme(strip.background = element_rect(fill=NA),
                        axis.title.y=element_text(size=-1),
                        axis.text.y = element_text(size=-1),
                        legend.position = 'none',
                        axis.text.x = element_text(angle=20, size=8),
                        axis.title.x=element_text(size=1.5))
  
per.nat.plot<-ggplot()+geom_col(data=aoo_nat[aoo_nat$name=='nat.area',], aes(x=percent.range/100, y=spFAC))+
  facet_wrap(~'Natural Range')+
  scale_y_discrete('')+
  ...
  theme_cowplot()+theme(strip.background = element_rect(fill=NA),
                        ...
                        axis.text.x = element_text(angle=20, size=8),
                        axis.title.x=element_text(size=1.5))
legend <- get_legend(sd.plot)
plot_grid(legend, plot_grid(sd.plot+theme(legend.position='none'),
                            CS.plot, per.nat.plot, rel_widths = c(1, .3,.3), nrow=1), 
          ncol=1, rel_heights=c(.1, 1))
ggsave('/home/tracidubose/rcs_results/sd_btwn_natExt_range.jpg',
       width=7, height=3.5)
```
